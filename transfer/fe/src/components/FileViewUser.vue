      ...
      <div class="content-section">
        <div class="main-content">
          <div v-if="activeTab === 'Basic'">
            ...
            <LanderPage :id="id" tab="Basic" :projectDetails="basicDetails" auth="user" />
          </div>
          <div v-if="activeTab === 'Specifications'">
            <h3>Specifications</h3>
            <LanderPage :id="id" tab="Specifications" auth="user" />
          </div>
          <div v-if="activeTab === 'Regions'">
            <h3>Regions</h3>
            <LanderPage :id="id" tab="Regions" auth="user" />
            
          </div>
          <div v-if="activeTab === 'Operators'">
            <h3>Operators</h3>
            <LanderPage :id="id" tab="Operators" auth="user"/>
          </div>
          <div v-if="activeTab === 'VersionQuality'">
            <h3>Version Quality</h3>
            <LanderPage :id="id" tab="VersionQuality" auth="user"/>
          </div>
          <div v-if="activeTab === 'VersionDetails'">
            <VersionBlockVue :project="this.name" auth="user" :id="id" />
          </div>
        </div>
        <StatusSliderVue :id="id" auth="user"/>
      </div>
    </div>
  </div>
</div>
...
export default {
  ...
  created() {
    ...
    console.log(routeQuery.usercount);
